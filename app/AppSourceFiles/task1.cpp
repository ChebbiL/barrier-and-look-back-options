...
int LBvegatest(){
	Lookback_option a(100, 140, 0.05, 0.4, 1, 0);
	ofstream fout;
	fout.open("Lookback_vega.csv"/*, ios::app*/);
	fout << "S0,vegaCF,vegaPW,vegaLR\n";
	for (int i = 80; i < 180; i++){
		a.input("St=" + to_string(i));
		fout << i << "," << a.vegaCF() << "," << a.vegaPW() << "," << a.vegaLR() << endl;
	}
	fout.close();
	return 0;
}
...
int write_csv(string pathToFile){
	...
	myfile << "iterations,EU th,EU th time,EU,EU time,Barrier th,Barrier th time,Barrier,Barrier time,Lookback_th,Lookback_th time,Lookback,Lookback time\n";
	Black_Scholes s1(100, 100, 0.05, 0.4, 1, 0);
	Barrier_option s2(100, 100, 0.05, 0.4, 1, 0);
	Lookback_option s3(100, 100, 0.05, 0.4, 1, 0 );
	...
	for (int i = 1; i<1000001; i *= 10){
		for (int j = 0; j<1000; j += 1){
			...
			double EU_th = s1.CF();
			clock_t end = clock();
			double EU_th_time = double(end - begin) / CLOCKS_PER_SEC;
			begin = clock();
			double EU = s1.OP();
			end = clock();
			double EU_time = double(end - begin) / CLOCKS_PER_SEC;
			begin = clock();
			double Barrier_th = s2.CF();
			end = clock();
			double Barrier_th_time = double(end - begin) / CLOCKS_PER_SEC;
			double Barrier = s2.OP();
			end = clock();
			double Barrier_time = double(end - begin) / CLOCKS_PER_SEC;
			double Lookback_th = s3.CF();
			end = clock();
			double Lookback_th_time = double(end - begin) / CLOCKS_PER_SEC;
			double Lookback = s3.OP();
			end = clock();
			double Lookback_time = double(end - begin) / CLOCKS_PER_SEC;

			myfile << i << "," << EU_th << "," << EU_th_time;
			myfile <<  "," << EU << "," << EU_time;
			myfile <<  "," << Barrier_th << "," << Barrier_th_time;
			myfile << "," << Barrier << "," << Barrier_time;
			myfile << "," << Lookback_th << "," << Lookback_th_time;
			myfile << "," << Lookback << "," << Lookback_time << endl;
		}
	}
