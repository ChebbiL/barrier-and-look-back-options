...
\textbf{\textit{ rand} generated U + CLT} & 6.28 e-5 & 0.999916 & 8.751 \\  \hline
...
\textbf{ linear congruential generator + CLT} & 8.87 e-5& 0.999189&23.96\\ \hline
\textbf{linear congruential generator + Box-Muller} & 1.34 e-4 & 1.000405&0.905\\ \hline
\textbf{ linear congruential generator + Marsagilia} & -8.517 e-5 & 1.000525&0.936\\ \hline
\textbf{\textit{Randon} }& cell8 & cell9&..\\ \hline

\end{tabular}
\end{center}

As shown in the table above, when using CLT generating standard normal distribution, the time needed is significantly longer than the rest methods. **More
...
\begin{align*}
\gamma_{DOC} =& {\phi(d_2) \over S_0\sigma\sqrt{T}}- \left({B \over S_0}\right)^{ 2\upsilon / \sigma^2}({4\upsilon^2 +2\upsilon \sigma^2\over S_0^2\sigma^4}C_{BS}(B^2 / S_0, K) + \gamma_{bs} - {4\upsilon\delta_{bs} \over S_0\sigma^2})
\end{align*}

\begin{align*}
\nu_{DOC} = S_0\phi(\frac{log({S_0 \over K}) + (r + {\sigma^2 \over 2})T}{\sigma\sqrt{T}})\sqrt{T}-({B \over S_0})^{2\upsilon \over \sigma^2}(\nu_{bs} - {4rC_{bs}({B^2\over S_0}, K)log({B\over S_0}) \over \sigma^3} )
\end{align*}
