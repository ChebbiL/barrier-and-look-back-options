...
\\[3cm] Authors:\\
%Tresnia Berah\\
Thomas Espel\\ Konstantin Kulak\\ Callum MacIver\\Zhengtian Qiu\\Vera Zhang }
...
$$Vega = \frac{\partial C}{\partial \sigma}=E[e^{-rT}(S_T-K)^+\left(\frac{Z^2-1}{\sigma}-Z\sqrt{T}\right)]$$

By pathwise methods, the greeks are given as following\footnote{For gamma, there are two different methods.}, with $Z~N(0,1)$:

$$Delta=\frac{\partial C}{\partial S_0}=E[e^{-rT}\frac{S_T}{S_0}]$$

$$Gamma_{LRPW} = \frac{\partial^2 C}{\partial  S_0^2 }=E[e^{-rT}K\frac{Z}{S_0^2\sigma\sqrt{T}}]$$

$$Gamma_{PWLR} = \frac{\partial^2 C}{\partial  S_0^2 }=E[e^{-rT}\frac{S_T}{S_0^2}\left(\frac{Z}{\sigma\sqrt{T}}-1\right)]$$

$$Vega = \frac{\partial C}{\partial \sigma}=E[e^{-rT}S_T(\sqrt{T}Z-\sigma T)]$$
